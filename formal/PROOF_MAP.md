...
| 30 | [`OccurrenceCertificates.lean`](ThreeNPlusMinusOne/OccurrenceCertificates.lean) | Data-level finite WIN/LOSS certificates retain branch choices, expose stored legal children/common grandchildren, and support a well-founded certified-forest replacement relation whose erasure preserves the logical rank | A concrete preinstalled certified forest and its semantic route invariant |
| 31 | [`HighReturnProvenance.lean`](ThreeNPlusMinusOne/HighReturnProvenance.lean) | The safe retained-pair case of the long `v≥7` return: exact retained `u,c` WIN tokens become exact lower `p,b` descendants with a strict forest-rank payment, embedded as two legal equal-base outer-forest occurrence steps | Entry, carry, no-reseed, and semantic `Bad` preservation proof that every high-return visit has that retained pair |
| 32 | [`CertifiedHighReturnProvenance.lean`](ThreeNPlusMinusOne/CertifiedHighReturnProvenance.lean) | The same safe `v≥7` pair payment with data-level retained certificates, including two equal-base data-level occurrence macro steps that erase to the earlier boundary | Entry, carry, and preinstallation of the incoming certificate pair |
| 33 | [`ProofPathProvenance.lean`](ThreeNPlusMinusOne/ProofPathProvenance.lean) | First-exit proof-tree provenance: a legal path is covered by a retained WIN tree or reveals a lower selected LOSS token, and the latter becomes an immediate outer-forest step; finite alternating paths admit compatible (not necessarily rank-compatible) trees | A finite preinstalled forest covering all zero-rank routes, and no-reseed transport across it |
| 34 | [`CertifiedProofPathProvenance.lean`](ThreeNPlusMinusOne/CertifiedProofPathProvenance.lean) | Data-level first-exit provenance: coverage follows retained certificate choices, while an exit yields a lower stored LOSS certificate and immediate data-level macro replacement | A finite preinstalled certificate forest covering all relevant routes |
| 35 | [`OccurrenceRefinement.lean`](ThreeNPlusMinusOne/OccurrenceRefinement.lean) | Equal-rank no-reseed boundary and a conditional no-DRAW theorem over occurrence-carrying configurations | Concrete semantic `lift` and `progress` fields |
| 36 | [`CertifiedOccurrenceRefinement.lean`](ThreeNPlusMinusOne/CertifiedOccurrenceRefinement.lean) | Data-level macro rank, well-foundedness, conditional no-DRAW theorem, and erasure of each step into the logical occurrence refinement | A concrete data-level semantic `lift` and `progress` invariant |
| 37 | [`Refinement.lean`](ThreeNPlusMinusOne/Refinement.lean) | `DrawMacroRefinement -> no Draw` and hence finite resolution | A value of `DrawMacroRefinement` |
| 38 | [`Termination.lean`](ThreeNPlusMinusOne/Termination.lean) | Transport of resolution and finite optimal proof steps back to the original odd-state game | Unconditional termination until the refinement exists |
