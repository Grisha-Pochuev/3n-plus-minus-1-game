...
| 22 | [`HighValuationReturn.lean`](ThreeNPlusMinusOne/HighValuationReturn.lean) | Conditional exact-coordinate bridge: an even scaled tail product and the signed relation force the returned `Q_{v-1}` coordinate | Derive those two hypotheses from the concrete signed valuation |
