...
| A legal finite path starting at a retained WIN tree either stays covered by that tree or exposes a strictly lower selected LOSS witness; in the latter case the witness is immediately a legal outer-forest occurrence replacement | High-return lifecycle handoff, Section 3 | **PROVED** in `ProofPathProvenance.lean`; carrying the forest after the first exit and excluding later reseeding remain open |
| A finite alternating route equipped with finite WIN/LOSS witnesses can be realized by a compatible finite proof tree that covers exactly that route | High-return lifecycle handoff, Section 7 | **PROVED** in `ProofPathProvenance.lean`; the compatible tree may be taller than a previously retained tree, so this theorem provides no replacement or rank decrease |
| In the long high-return row `v≥7`, if the incoming forest already retains exact WIN occurrences at the returned state and its contracting child, the two exact common-grandchild occurrences replace them with both heights and the forest rank strictly lower; the replacement is also two explicit equal-base `OccurrenceMacroStep.outerForest` steps | Sections 100–103, 129, 136 | **PROVED CONDITIONALLY** in `HighReturnProvenance.lean`; preservation of a semantic `Bad` predicate, and the entry, carry, and no-reseed conditions needed to establish that incoming pair, remain open |
...
one-worker GitHub Actions run #29. The active pull-request workflow has exactly
