...
| The returned contracting side selects the Section 34 lower child through `B` at `v=4` and through `A` at `v≥5`; any exact valuation-two signed transition from a DRAW parent packages its actual carrier as the corresponding `DrawObligation`, which still exposes a DRAW lift; under the Section 24 A-selecting guard a shared WIN child is thereby a concrete DRAW-to-WIN boundary.  The guard is now derived uniformly for the high-return long range `v≥6` | Section 35 | **FORMALIZED; ONE-WORKER KERNEL CHECK PENDING** in `HighValuationTransfer.lean`; deriving the concrete high-return valuation-two tail and the final common-child WIN witness remains open |
