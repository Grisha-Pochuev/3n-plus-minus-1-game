...
| Given the scaled signed relation and an even tail product, the returned exponent-three source is exactly the lower-exponent `Q` coordinate | Section 33 | **PROVED** in `HighValuationReturn.lean`; deriving the scaled relation and evenness from the concrete valuation remains open |
